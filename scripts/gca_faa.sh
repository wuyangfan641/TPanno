...
while IFS=$'\t' read -r species gca || [[ -n "$species" ]]; do
    ...
    output_faa="$output_dir/${safe_species_name}.faa"

    # 检查目标文件是否已存在
    if [ -f "$output_faa" ]; then
        echo "  文件 '$output_faa' 已存在，跳过。"
        continue
    fi

    # 使用NCBI Datasets API v2构建下载链接
    download_url="https://api.ncbi.nlm.nih.gov/datasets/v2/genome/accession/${gca}/download?include_annotation_type=PROT_FASTA"
    ...
    if wget --header="Accept: application/zip" -qO "$zip_file" "$download_url"; then
        ...
        # 在解压文件中查找 .faa 文件
        faa_file=$(find "$temp_dir/ncbi_dataset/data" -type f -name "*.faa" 2>/dev/null)

        if [[ -f "$faa_file" ]]; then
            # 移动并重命名文件
            mv "$faa_file" "$output_faa"
            echo "  成功: 基因组已保存为 '$output_faa'"
        else
